...
//    			没有radis缓存,查询前十条数据，存入redis,剩下的由AJAX加载出来
    	...
    	{
			//定义两个键
			$shopkeys='LIST:shopkeys';
			$shophashs='HSAH:shopContents:';
    		//判断radis中是否有商品信息
    		if(Redis::exists($shopkeys))
    		{
    			//获取所有商品的ID
    			$AllId=Redis::lrange($shopkeys,0,-1);
    			foreach($AllId as $k=>$v)
    			{
    				//把商品内容和商品ID相关联
    				$shopContent[]=Redis::hgetall($shophashs.$v);
    			}
    		}else
    		{
//    			没有radis缓存,查询前十条数据，存入redis,剩下的由AJAX加载出来
    			...
    			//存入redis
				 ...
				 {
				 	//将商品ID存入$shopkey中
				 	Redis::rpush($shopkeys,$v['id']);
				 	//将商品内容放入shophash中
				 	Redis::hmset($shophashs.$v['id'],$v);
				 }
    		}
			$shopContent=Goods::limit(10)->get()->toArray();
			return view('home.index',['shopContent'=>$shopContent]);
    	}
